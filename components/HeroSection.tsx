...
const HeroSection = () => {
    ...
    return (
        <section id="hero" className="relative min-h-[90vh] flex flex-col items-center justify-center overflow-hidden px-4 sm:px-6 pt-24 pb-16 text-center">
            <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
                <div className="absolute top-1/4 left-1/4 w-72 h-72 md:w-96 md:h-96 rounded-full opacity-10 bg-[radial-gradient(circle,var(--color-primary),transparent_70%)] animate-[float_8s_ease-in-out_infinite]" />
                <div className="absolute bottom-1/4 right-1/4 w-64 h-64 md:w-80 md:h-80 rounded-full opacity-8 bg-[radial-gradient(circle,var(--color-primary),transparent_70%)] animate-[float_10s_ease-in-out_infinite_3s]" />
            </div>
            ...
            <h1 className="relative font-bold leading-[1.1] mb-6 max-w-4xl font-serif text-[clamp(2.5rem,8vw,5rem)]">
                Talk to your books.
                <br />
            ...
            <div className="relative flex flex-col items-center gap-6 mb-12 w-full">
                <div className="flex items-center gap-4 sm:gap-6 bg-card border border-border rounded-2xl px-4 sm:px-6 py-4 shadow-2xl">
                    <Waveform active={true} className="hidden sm:flex" />
                    <button
                        onClick={handleAction}
                        aria-label="Start voice conversation"
                        className="relative w-14 h-14 rounded-full flex items-center justify-center transition-all duration-200 bg-primary hover:scale-105 active:scale-95 focus:outline-none focus:ring-4 focus:ring-primary/20"
                    >
                        <PulseRing />
            ...
            <div className="relative flex flex-col sm:flex-row justify-center items-center gap-4">
                <button
                    onClick={handleAction}
                    className="flex items-center gap-2 bg-primary text-primary-foreground px-8 py-3.5 rounded-full font-medium hover:opacity-90 transition-opacity text-sm shadow-lg focus:ring-4 focus:ring-primary/20"
                >
                    {isSignedIn ? "Add your book" : "Start for free"} <ArrowRight size={15} />
                </button>
                <button
                    onClick={() => setShowVideo(true)}
                    className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors px-6 py-3.5 focus:outline-none focus:text-primary"
                    aria-label="Watch 18-second demo video"
                >
            ...
            {showVideo && (
                ...
                >
                    <div ref={modalRef} className="relative w-full max-w-4xl bg-black rounded-2xl overflow-hidden shadow-2xl focus:outline-none" tabIndex={-1}>
                        <button
