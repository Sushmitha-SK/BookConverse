...
const Transcript = ({ messages, currentMessage, currentUserMessage }: TranscriptProps) => {
    ...
    if (isEmpty) {
        return (
            <div className="flex flex-col items-center justify-center h-64 text-center p-6 border-2 border-dashed border-border rounded-3xl bg-card/50">
                <div className="w-16 h-16 bg-secondary rounded-full flex items-center justify-center mb-4">
                    <Mic className="size-6 text-primary" />
                </div>
                <h3 className="text-foreground font-semibold">Ready to read?</h3>
    ...
    const MessageBubble = ({ content, role, isStreaming }: { content: string, role: 'user' | 'assistant', isStreaming?: boolean }) => (
        <div className={`flex items-end gap-3 ${role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {role === 'assistant' && (
            ...
            >
                {content}
                {isStreaming && <span className="inline-block w-1.5 h-4 ml-1 bg-primary align-middle animate-blink" />}
            </div>
        </div>
    );
